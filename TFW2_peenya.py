...
test_file_path = os.path.join(MAINFOLDER, "24hrData", "Dailydata_TFW2peenya.xlsx")  # DAILY DATA
excel_file_path = os.path.join(MAINFOLDER, "Breakdownrecords_TFW2peenya.xlsx")  # Recording excel for BD
folderpath = os.path.join(MAINFOLDER, "TemporaryData")  # Temporary dump files collector
threshold_file_path = os.path.join(MAINFOLDER,"Thresholds_TFW2peenya.xlsx") 
...
st.markdown(
    ...
    <div style="display: flex; align-items: center;">
        ...
        <span style="font-size: {title_font_size}px; font-weight: bold;">Breakdown Predictor-TFW 2(Peenya)</span>
    </div>
...
def process_all_files():
    ...
    assets_list = [
        "TFW-2 MDE", "TFW-2 MNDE",
            "TFW-2 Spindle DE ", "TFW-2 Spindle NDE"]
...
def predict_time(test_file_path):
    ...
    def preprocess_test_data(X_test):
        scaler = joblib.load(os.path.join(model_folder_path, 'scaler_TFW2_time.pkl'))
        X_test_scaled = scaler.transform(X_test)
        return X_test_scaled

    def predict_time_to_breakdown(X_test_scaled):
        model = load_model(os.path.join(model_folder_path, 'time_TFW2.h5'))
